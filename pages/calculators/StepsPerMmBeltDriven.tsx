...
import { beltpresetsOptions, microsteppingOptions, motorStepAngleOptions } from './options';

export default function StepsPerMmBeltDrivenCalculator() {
  ...
  const [BeltPitch, setBeltPitch] = useState<number | null>(null);
  const [BeltPresets, setBeltPresets] = useState<number | null>(null);
  const [PulleyToothCount, setPulleyToothCount] = useState<number | null>(null);
  const [result, setResult] = useState<string | null>(null);

  const calculateBeltDriven = () => {
    if (MotorStepAngle && DriverMicrostepping && BeltPitch && PulleyToothCount) {
      const calcResult = (
        (((360 / MotorStepAngle) * DriverMicrostepping) / (BeltPitch * PulleyToothCount))
      ).toPrecision(8);
  ...
  return (
    <>
      <NextSeo
        title="Steps per mm belt driven system"
      ...
      <CalculatorContainer>
        <CalculatorForm>
          ...
          <label htmlFor="BeltPitch">Belt Pitch (in mm):</label>
          <input
            type="number"
            id="BeltPitch"
            value={BeltPitch ?? ''}
            onChange={(e) => setBeltPitch(Number(e.target.value))}
            required
          />
          ...
          <label htmlFor="PulleyToothCount">Pulley Tooth Count:</label>
          <input
            type="number"
            id="PulleyToothCount"
            value={PulleyToothCount ?? ''}
            onChange={(e) => setPulleyToothCount(Number(e.target.value))}
            required
          />
          <CalculateButton onClick={calculateBeltDriven}>Calculate</CalculateButton>
