...
export default function BatteryBackupCalculator() {
  ...
  const handleCalculate = () => {
    if (batteryCapacity && batteryVoltage && numBatteries && totalLoad) {
      const calcResult = (
        (batteryCapacity * batteryVoltage * numBatteries * 0.75) / totalLoad
      ).toPrecision(3);
      setResult(`Battery Run Time: ${calcResult} hours`);
    } else {
  ...
  return (
    ...
    <CalculatorContainer>
    <CalculatorForm>
        <h2>Battery Backup Calculator</h2>
        ...
        required
        />
        <label htmlFor="numBatteries">Number of Batteries:</label>
        <input
        type="number"
        id="numBatteries"
        value={numBatteries ?? ''}
